...
const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!auth.loading && (!auth.user || auth.user.role !== 'patient')) {
      router.push('/');
    }
  }, [authContext, router]);
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p className="text-gray-700 dark:text-gray-300 mt-2">
                  <span className="font-medium">Role:</span> Patient
                </p>
              </div>
            </div>
            ...
            {user.doctor && (
              <div className="mt-8">
                <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">Assigned Doctor</h3>
                <div className="flex items-center">
                  <div className="w-16 h-16 rounded-full bg-blue-500 flex items-center justify-center text-white text-2xl font-bold">
                    {user.doctor.name.charAt(0)}
                    {user.doctor.lname.charAt(0)}
                  </div>
                  <div className="ml-4">
                    <p className="text-lg font-semibold text-gray-800 dark:text-gray-200">
                      Dr. {user.doctor.name} {user.doctor.lname}
                    </p>
                    <p className="text-gray-600 dark:text-gray-400">{user.doctor.email}</p>
                    {user.doctor.licenseNumber && (
                      <p className="text-gray-600 dark:text-gray-400">
                        License Number: {user.doctor.licenseNumber}
                      </p>
                    )}
                  </div>
                </div>
